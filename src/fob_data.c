...
static bool is_same_uuid(lock_pairing* existing_pairing, lock_pairing* new_pairing) {
    for(int i = 0; i < NUKI_UUID_LEN; i++) {
        if(existing_pairing->pairing.lock_uuid[i] != new_pairing->pairing.lock_uuid[i]) {
            return false;
        }
    }
    return true;
...
static void prepare_writing_flash(fob_data_writing_context* ctx) {
    ...
    for(uint16_t i = 0; i < MAX_PAIRINGS; i++) {
        if(is_same_uuid(&fob_data_ram->paired_locks[i], &ctx->data)) {
            pairing_slot = i;
        }
    }
    fob_data_ram->paired_locks[pairing_slot] = ctx->data;
...
static void write_flash_page(uint32_t address, fob_data_writing_context* ctx) {
    ret_code_t error = nrf_fstorage_write(&fstorage, address, ctx->page_buffer, sizeof(fob_data), ctx);
    if(error != NRF_SUCCESS) {
        ctx->write_state = WS_ERROR;
    }
}
