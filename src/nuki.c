...
static uint16_t encrypt_payload(nuki_key* key, uint8_t* output_buffer, uint8_t* pdata_unencrypted, uint16_t pdata_length) {
    if(pdata_length > MAX_PDATA_LENGTH) { return 0; }
    uint8_t adata[ADATA_LENGTH];
    randombytes(adata, 24); //generate nonce
    write_uint32LE(adata, key->authorization_id, 24);
...
static bool decrypt_challenge(nuki_key* key, uint8_t* out_nonce, uint8_t* encrypted_challenge) {
    ...
    if(read_uint16LE(decrypted_message, crc_offset) != crc || read_uint32LE(decrypted_message, 0) != key->authorization_id) {
        return false;
    }
...
static void calculate_authenticator(nuki_key* key, uint8_t* output_buffer, uint8_t* message, uint16_t message_length) {
    HMAC_SHA256_compute(message, message_length, key->shared_secret, 32, output_buffer);
}

uint16_t create_challenge_payload(nuki_key* key, uint8_t* output_buffer) {
    uint8_t pdata_unencrypted[CHALLENGE_CMD_PDATA_LENGTH];
    write_uint32LE(pdata_unencrypted, key->authorization_id, 0);
    write_uint16LE(pdata_unencrypted, request_data_cmd, 4);
    write_uint16LE(pdata_unencrypted, challenge_cmd, 6);
    return encrypt_payload(key, output_buffer, pdata_unencrypted, CHALLENGE_CMD_PDATA_LENGTH);
}
...
uint16_t create_lock_action_payload(nuki_lock_action_context* lock_action_ctx, uint8_t* output_buffer, uint8_t* encrypted_challenge) {
    ...
    bool decrypted = decrypt_challenge(&lock_action_ctx->key, &pdata_unencrypted[12], encrypted_challenge);
    if(!decrypted) { return 0; }
    uint8_t flags = 0;
    write_uint32LE(pdata_unencrypted, lock_action_ctx->key.authorization_id, 0);
    write_uint16LE(pdata_unencrypted, lock_action_cmd, 4);
    write_uint8LE(pdata_unencrypted, lock_action_ctx->lock_action, 6);
    write_uint32LE(pdata_unencrypted, lock_action_ctx->app_id, 7);
    write_uint8LE(pdata_unencrypted, flags, 11);

    return encrypt_payload(&lock_action_ctx->key, output_buffer, pdata_unencrypted, LOCK_ACTION_PDATA_LENGTH);
}
...
uint16_t create_write_public_key_payload(nuki_pairing_context* pairing_ctx, uint8_t* output_buffer, uint8_t* received_data) {
    ...
    crypto_box_keypair(pairing_ctx->public_key_fob, pairing_ctx->private_key_fob);
    memcpy(pairing_ctx->public_key_nuki, received_public_key, crypto_box_PUBLICKEYBYTES);

    uint16_t command_length = 36;
    write_uint16LE(output_buffer, public_key_cmd, 0);
    memcpy(&output_buffer[2], pairing_ctx->public_key_fob, crypto_box_PUBLICKEYBYTES);
    crc_payload(output_buffer, command_length);
    return command_length;
}
...
uint16_t create_authorization_authenticator_payload(nuki_pairing_context* pairing_ctx, uint8_t* output_buffer, uint8_t* received_data) {
    ...
    crypto_scalarmult(dh_key, pairing_ctx->private_key_fob, pairing_ctx->public_key_nuki);
    unsigned char _0[16];
    memset(_0, 0, 16);
    const unsigned char sigma[17] = "expand 32-byte k";
    crypto_core_hsalsa20(pairing_ctx->key.shared_secret, _0, dh_key, sigma);
    ...
    memcpy(r, pairing_ctx->public_key_fob, crypto_box_PUBLICKEYBYTES);
    memcpy(&r[crypto_box_PUBLICKEYBYTES], pairing_ctx->public_key_nuki, crypto_box_PUBLICKEYBYTES);
    memcpy(&r[crypto_box_PUBLICKEYBYTES + crypto_box_PUBLICKEYBYTES], nonce, PAIRING_NONCEBYTES);
    uint8_t authenticator[32];
    calculate_authenticator(&pairing_ctx->key, authenticator, r, r_length);
...
uint16_t create_authorization_data_payload(nuki_pairing_context* pairing_ctx, uint8_t* output_buffer, uint8_t* received_data) {
    ...
    memcpy(&r[5], pairing_ctx->fob_name, 32);
...
uint16_t create_authorization_id_confirmation_payload(nuki_pairing_context* pairing_ctx, uint8_t* output_buffer, uint8_t* received_data) {
    ...
    pairing_ctx->key.authorization_id = read_uint32LE(received_data, 34);
    const uint16_t r_length = 36;
    uint8_t r[36];
    write_uint32LE(r, pairing_ctx->key.authorization_id, 0);
